...
def create_skybox(pipeline):
    ...
    gpuSky.texture = es.textureSimpleSetup(
        getAssetPath("fov90_frente.png"), GL_REPEAT, GL_REPEAT, GL_LINEAR, GL_LINEAR)
    ...
    gpuSecondSky.texture = es.textureSimpleSetup(
        getAssetPath("fov90_lado.png"), GL_REPEAT, GL_REPEAT, GL_LINEAR, GL_LINEAR)
    ####################################################################################################
    ...
    secondSky.transform = tr.matmul([tr.translate(0.99, 0, 0.30), tr.rotationX(math.pi/2), tr.rotationY(math.pi/2), tr.uniformScale(2)])
    secondSky.childs += [gpuSecondSky]

    newSkybox = sg.SceneGraphNode("secondSky")
    newSkybox.transform = tr.identity()
    newSkybox.childs += [skybox, secondSky]
    ############################################################
    return newSkybox
