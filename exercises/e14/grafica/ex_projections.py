...
if __name__ == "__main__":
    ...
    camera_target = np.array([5.0, 5.0, 5.0])
    camera_direction = camera_target - camera_position

    rho, phi, thetha = 10, 1, 1
    ...
    while not glfw.window_should_close(window):
        ...
        if (glfw.get_key(window, glfw.KEY_A) == glfw.PRESS):
            camera_position += 0.03*camera_direction

        if (glfw.get_key(window, glfw.KEY_Z) == glfw.PRESS):
            camera_position -= 0.03*camera_direction

        camera_target = camera_position + np.array([rho*np.cos(thetha)*np.sin(phi), rho*np.cos(phi), rho*np.sin(thetha)*np.sin(phi)])
